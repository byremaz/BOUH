 ...
 * Caller: appointmentsController. Uses appointmentRepo, DoctorRepo, ChildRepo to read Firestore and build response DTOs.
 */
@Service
public class AppointmentsService {
    ...
    /**
     * Get upcoming appointments for the given caregiverId: query appointments with date >= today (no status filter).
     * status in DB is for attendance only (حضر / لم يحضر). Resolve doctor, child name, time slot for each.
     */
    public List<upcomingAppointmentDto> getUpcomingAppointments(String caregiverId)
        ...
        for (appointmentDto doc : docs) {
            // status is for attendance only (حضر / لم يحضر), not for filtering upcoming — show all with date >= today
